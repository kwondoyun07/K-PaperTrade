...
# 진입 위치 규율의 숫자는 8/31~9/7 실측이다. 유효 표본이 6거래일뿐이라 통계적으로
# 유의하지 않다 — 다만 추격 매수 경향은 8/26과 9/7 두 시점에서 독립적으로 관찰됐고
...
def build_prompt(rows: list[str], holdings: dict[str, int], track: str = "") -> str:
    ...
    return (
        ...
        "  강세 '확인'은 매수 근거가 되지만 이미 올라버린 가격 자체는 근거가 아니다.\n"
        "  확신이 같다면 눌림(고저위치 낮음·VWAP 근처)까지 기다려 HOLD하라 — 놓친 상승은\n"
        "  기회비용이지만 고가 추격은 실제 손실이다.\n"
        "- 단, 이 규율은 매수에만 적용한다. 손절·악재 청산은 가격 위치와 무관하게 즉시 하라.\n\n"
